...
//}
//
//IPage& IPage::operator= (const IPage& other) {
//	_client = other._client;
//	_table = other._table;
//	_functions = other._functions;
//	return *this;
...
//
//Memento* IPage::save() {
//	return new IPageMemento(_client, _table, _functions);
//}
...
void Interface::moveWithinGridAndExecuteFunction(Table& _grid) {
...
Point Interface::moveWithinGrid(Table& _grid) {
	unsigned row = 0, col = 0;
	_grid[0][0]->setPointerTo();
	bool flagContinue = true;
	while (flagContinue) {
...
void Interface::menuPage() {
	system("cls");
	_state._menuPage = Table(1, 5, 2, 4);
...
void Interface::schedulePage() {
	...
	//while (flagContinue) {
		...
		moveWithinGridAndExecuteFunction(schedulePage);
	//}
}
